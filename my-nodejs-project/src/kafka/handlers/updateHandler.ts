...
export const updateHandler = async (actionConsumer: Consumer) => {
  // await wait();
  const { actionId = "", skip = 0 } = await actionService.getRunningAction();
  ...
  const updateObject = actData.reduce((acc: any, curr: [string, any]) => {
